//https://git.oschina.net/fcten/libjson
/**
 *
 *
 void json_demo()
{
    ...
    json_print(obj, &p, &len);

    printf("%.*s\n", (int)(sizeof(buf)-len), buf);

    json_object_t * new_obj = json_duplicate_object(obj);

    json_delete_object(obj);

    p = buf;
    len = sizeof(buf);
    json_print(new_obj, &p, &len);

    printf("%.*s\n", (int)(sizeof(buf)-len), buf);

    json_delete_object(new_obj);
    printf("\n");
}
